...
/*  Approach -
    ...
    a _ ,  b _ , c _ , ... (each set will have 25 words) and there are 26 such sets for total 650 words
    1 - 25,26 - 50 , ...

    Therfore we find the set by doing s[0]*25 Add s[1] for the exact index
    Since we have to skip words with equal letters if 1st letter > 2nd letter if subtract 1 from its rank
...
void solve()
{
    ...
    debug(temp);
    rank += (int)(s[0] - 'a') * 25; // calculates which set it belongs, 1, 25, 50 etc
