 ...
 *
 * @brief-
 * Arrange the given amount of coins into boxes (1 coin in 1 box)
 * 1st row - 1 box, 2nd row - 2 boxes, 3rd row - 3 boxes and so on...
 * Return the number of rows that are completely filled
 *
 ** approach-
    since ith row contains i boxes, total no number of coins for i rows is i*(i+1)/2;
    therefore we binary search to find greatest i such that sum = i*(i+1)/2 is <= n
    (sum can be less since next row can be partially filled)
 */
// classes & functions
class Solution
{
public:
    int arrangeCoins(int n)
    {
        ...
        int res = 0;
        ll loSum = (lo * (lo + 1)) / 2, hiSum = (hi * (hi + 1)) / 2;
        if (loSum <= n)
        {
            res = lo;
        }
